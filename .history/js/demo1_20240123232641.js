 ...
 */
{
    ...
    const timeline_sections = Array.from(document.querySelectorAll('.timeline'));
    ...
    function initilaCheckElementsOnScroll() {
        timeline_sections.forEach((section, index) => {
            const bounds = section.getBoundingClientRect();
            const middleOfElement = bounds.top + bounds.height / 2;
            const middleOfViewport = window.innerHeight / 2;
            const isInMiddle = middleOfElement > middleOfViewport * 0.4 && middleOfElement < middleOfViewport * 0.6;
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        timeline_sections.forEach(section => allentries.push(new Entry(section)));
